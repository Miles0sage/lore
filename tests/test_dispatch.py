...
def _reset_all():
    dispatch._failure_counts.clear()
...
class TestCircuitBreaker:
    def test_circuit_closed_initially(self):
        for model in dispatch._TIER_ORDER:
            assert not dispatch._is_circuit_open(model)

    def test_circuit_opens_after_threshold(self):
        model = "gpt-4.1"
        for _ in range(dispatch._FAILURE_THRESHOLD):
            dispatch._record_failure(model)
        assert dispatch._is_circuit_open(model)

    def test_circuit_resets_on_success(self):
        model = "gpt-4.1"
        for _ in range(dispatch._FAILURE_THRESHOLD):
            dispatch._record_failure(model)
        dispatch._record_success(model)
        assert not dispatch._is_circuit_open(model)
    ...
    def test_get_circuit_status_one_open(self):
        for _ in range(dispatch._FAILURE_THRESHOLD):
            dispatch._record_failure("deepseek-chat")
        status = dispatch.get_circuit_status()
